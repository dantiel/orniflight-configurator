...
configuration_backup = (callback) ->
    ...
    fetch_specific_data = ->
        ...
        fetch_specific_data_item = ->
            if fetchingProfile < CONFIG.numProfiles
                MSP.send_message profileSpecificData[codeKey], false, false, ->
                    codeKey++
    ...
    fetch_unique_data = ->
        ...
        fetch_unique_data_item = ->
            if codeKey < uniqueData.length
                MSP.send_message uniqueData[codeKey], false, false, ->
                    codeKey++
                    fetch_unique_data_item()
                    return
            else
        ...
        MSP.promise(MSPCodes.MSP_ADVANCED_CONFIG).then(->
    ...
    save = ->
        ...
        if GUI.Mode == GUI_Modes.NWJS
            ...
            input.onchange = ->
                if @value
                    fs.writeFileSync @value, serialized_config_object
                    console.log 'Backup file path: ' + @value
        ...
        chrome.fileSystem.chooseEntry {
            ...
            chrome.fileSystem.getDisplayPath chosenFileEntry, (path) ->
                ...
                return
            chrome.fileSystem.getWritableEntry chosenFileEntry, (fileEntryWritable) ->
                chrome.fileSystem.isWritableEntry fileEntryWritable, (isWritable) ->
                    if isWritable
                        chosenFileEntry = fileEntryWritable
                        blob = new Blob([ serialized_config_object ], type: 'text/plain')
                        chosenFileEntry.createWriter ((writer) ->
...
configuration_restore = (callback) ->

    process_configuration = (configuration) ->
        if typeof configuration.generatedBy != 'undefined' and compareVersions(configuration.generatedBy, CONFIGURATOR.backupFileMinVersionAccepted)
            if !compareVersions(configuration.generatedBy, '1.14.0') and !migrate(configuration)
                GUI.log i18n.getMessage('backupFileUnmigratable')
                return
            if configuration.FEATURE_CONFIG and configuration.FEATURE_CONFIG.features and configuration.FEATURE_CONFIG.features._featureMask
    ...
    compareVersions = (generated, required) ->
        if generated == undefined
            return false
        semver.gte generated, required
    ...
    migrate = (configuration) ->
        ...
        if !compareVersions(migratedVersion, '0.61.0')
            ...
            if !configuration.PIDs and configuration.PID
                configuration.PIDs = configuration.PID
                configuration.PID = controller: 0
        ...
        if configuration.apiVersion == undefined
            ...
            # a guess that will satisfy the rest of the code
        # apiVersion previously stored without patchlevel
        if !semver.parse(configuration.apiVersion)
            configuration.apiVersion += '.0'
            if !semver.parse(configuration.apiVersion)
                return false
        if compareVersions(migratedVersion, '0.63.0') and !compareVersions(configuration.apiVersion, '1.7.0')
            ...
            while portIndex < configuration.SERIAL_CONFIG.ports.length
                ...
                switch oldPort.scenario
                    # MSP, CLI, TELEMETRY, SMARTPORT TELEMETRY, GPS-PASSTHROUGH
                    ...
                    when 11
                        # MSP, CLI, BLACKBOX, GPS-PASSTHROUGH
                        newPort.functions.push 'MSP'
                        newPort.functions.push 'BLACKBOX'
                ports.push newPort
                portIndex++
            configuration.SERIAL_CONFIG = ports: ports
            GUI.log i18n.getMessage('configMigratedTo', [ migratedVersion ])
            appliedMigrationsCount++
        if compareVersions(migratedVersion, '0.63.0') and !compareVersions(configuration.apiVersion, '1.8.0')
        ...
        if semver.lt(migratedVersion, '0.66.0')
            ...
            while profileIndex < configuration.profiles.length
                ...
                while i < configuration.profiles[profileIndex].ServoConfig.length
                    ...
                    if servoConfig[i].rate < -100 or servoConfig[i].rate > 100
            ...
            GUI.log i18n.getMessage('configMigratedTo', [ migratedVersion ])
            appliedMigrationsCount++
        if semver.lt(configuration.apiVersion, '1.14.0') and semver.gte(CONFIG.apiVersion, '1.14.0')
            ...
            GUI.log i18n.getMessage('configMigratedTo', [ migratedVersion ])
            appliedMigrationsCount++
        if compareVersions(migratedVersion, '0.66.0') and !compareVersions(configuration.apiVersion, '1.14.0')
            ...
            GUI.log i18n.getMessage('configMigratedTo', [ migratedVersion ])
            appliedMigrationsCount++
        if compareVersions(migratedVersion, '0.66.0') and !compareVersions(configuration.apiVersion, '1.15.0')
        ...
        upload = ->
            ...
            upload_specific_data = ->
                ...
                upload_using_specific_commands = ->
                    MSP.send_message profileSpecificData[codeKey], mspHelper.crunch(profileSpecificData[codeKey]), false, ->
                        codeKey++
                        if codeKey < profileSpecificData.length
                            upload_using_specific_commands()
                        else
            ...
            upload_unique_data = ->
                ...
                send_unique_data_item = ->
                    if codeKey < uniqueData.length
                        MSP.send_message uniqueData[codeKey], mspHelper.crunch(uniqueData[codeKey]), false, ->
                            codeKey++
                            send_unique_data_item()
                            return
                    else
    ...
    if GUI.Mode == GUI_Modes.NWJS
        ...
        input.onchange = ->
            ...
            if content.length > 1048576
                ...
                return
            console.log 'Restore file path: ' + @value
