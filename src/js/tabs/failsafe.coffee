...
TABS.failsafe.initialize = (callback, scrollPosition) ->
    self = this
    ...
    load_rxfail_config = ->
        MSP.send_message MSPCodes.MSP_RXFAIL_CONFIG, false, false, if semver.gte(CONFIG.apiVersion, '1.41.0') then load_gps_rescue else get_box_names
        return
    ...
    process_html = ->
        ...
        if typeof RSSI_CONFIG.channel != 'undefined'
            ...
            # Aux channels start at 5 in backend so we have to substract 5
        modeIndex = 0
        ...
        $('input[type="radio"].procedure').change ->
            `var element`
            element = $(this)
        ...
        $('a.save').click ->
            ...
            save_feature_config = ->
                MSP.send_message MSPCodes.MSP_SET_FEATURE_CONFIG, mspHelper.crunch(MSPCodes.MSP_SET_FEATURE_CONFIG), false, if semver.gte(CONFIG.apiVersion, '1.41.0') then save_gps_rescue else save_to_eeprom
                return
            ...
            if semver.gte(CONFIG.apiVersion, '1.39.0')
                ...
                FAILSAFE_CONFIG.failsafe_switch_mode = if $('input[name="failsafe_kill_switch"]').is(':checked') then 1 else 0
            if semver.gte(CONFIG.apiVersion, '1.41.0')
                # Load GPS Rescue parameters
...
TABS.failsafe.cleanup = (callback) ->
    if callback
        callback()
    return
