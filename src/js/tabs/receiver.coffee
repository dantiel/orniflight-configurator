updateInterpolationView = ->
    ...
    if semver.gte(CONFIG.apiVersion, '1.42.0')
        if RX_CONFIG.rcSmoothingDerivativeCutoff == 0 or RX_CONFIG.rcSmoothingInputCutoff == 0
...
TABS.receiver.initialize = (callback) ->
    ...
    process_html = ->
        ...
        if semver.lt(CONFIG.apiVersion, '1.15.0')
            ...
            $('.deadband input[name="deadband"]').change(->
                tab.deadband = parseInt($(this).val())
                return
            ).change()
            $('.deadband input[name="yaw_deadband"]').change(->
        ...
        if semver.gte(CONFIG.apiVersion, '1.20.0')
            ...
            $('select[name="rcInterpolation-select"]').change(->
        ...
        aux_index = 1
        num_bars = if RC.active_channels > 0 then RC.active_channels else 8
        i = 0
        while i < num_bars
            name = undefined
            ...
            bar_container.append '                <ul>                    <li class="name">' + name + '</li>                    <li class="meter">                        <div class="meter-bar">                            <div class="label"></div>                            <div class="fill' + (if RC.active_channels == 0 then 'disabled' else '') + '">                                <div class="label"></div>                            </div>                        </div>                    </li>                </ul>            '
            i++
        ...
        meter_scale = 
            ...
            'max': 2200
        meter_fill_array = []
        $('.meter .fill', bar_container).each ->
            meter_fill_array.push $(this)
            return
        meter_label_array = []
        $('.meter', bar_container).each ->
        ...
        # trigger so labels get correctly aligned on creation
        ...
        # go out of bounds
        $('select[name="rcmap_helper"]').change ->
        ...
        $('a.sticks').click ->
            ...
            chrome.app.window.create '/tabs/receiver_msp.html', {
                ...
                createdWindow.contentWindow.setRawRx = (channels) ->
                    if CONFIGURATOR.connectionValid and GUI.active_tab != 'cli'
        ...
        if semver.gte(CONFIG.apiVersion, '1.40.0')
            ...
            rc_smoothing_protocol_e.change ->
            ...
            if RX_CONFIG.rcSmoothingDerivativeCutoff == 0
                ...
                $('.tab-receiver .rcSmoothing-derivative-cutoff').hide()
            $('select[name="rcSmoothing-input-derivative-select"]').change ->
                if $(this).val() == 0
                    $('.tab-receiver .rcSmoothing-derivative-cutoff').hide()
            ...
            rc_smoothing_input_type.change ->
                RX_CONFIG.rcSmoothingInputType = $(this).val()
                return
            rc_smoothing_input_type.val RX_CONFIG.rcSmoothingInputType
            if semver.gte(CONFIG.apiVersion, '1.42.0')
                $('select[name="rcSmoothing-input-manual-select"], select[name="rcSmoothing-input-derivative-select"]').change ->
                    if $('select[name="rcSmoothing-input-manual-select"]').val() == 0 or $('select[name="rcSmoothing-input-derivative-select"]').val() == 0
                        $('.tab-receiver .rcSmoothing-auto-smoothness').show()
                    else
                        $('.tab-receiver .rcSmoothing-auto-smoothness').hide()
                    return
                ...
                rc_smoothing_auto_smoothness.change ->
        ...
        $('select[name="rx_refresh_rate"]').change ->
            ...
            update_ui = ->
                ...
                widthScale = d3.scale.linear().domain([
                    samples - 299
                    samples
                ])
                heightScale = d3.scale.linear().domain([
                    800
                    2200
                ])
                update_receiver_plot_size()
                ...
                xAxis = d3.svg.axis().scale(widthScale).orient('bottom').tickFormat((d) ->
                    d
                )
                yAxis = d3.svg.axis().scale(heightScale).orient('left').tickFormat((d) ->
                    d
                )
                ...
                lines = data.selectAll('path').data(RX_plot_data, (d, i) ->
                    i
                )
            ...
            return
        ConfigStorage.get 'rx_refresh_rate', (result) ->
...
TABS.receiver.getRecieverData = ->
    MSP.send_message MSPCodes.MSP_RC, false, false
    return

TABS.receiver.initModelPreview = ->
    @keepRendering = true
    @model = new Model($('.model_preview'), $('.model_preview canvas'))
    @useSuperExpo = false
    ...
    if CONFIG.flightControllerIdentifier == 'CLFL' and semver.lt(CONFIG.apiVersion, '2.0.0')
        useOldRateCurve = true
    if CONFIG.flightControllerIdentifier == 'ORNI' and semver.lt(CONFIG.flightControllerVersion, '2.8.0')
        useOldRateCurve = true
    @rateCurve = new RateCurve(useOldRateCurve)
    $(window).on 'resize', $.proxy(@model.resize, @model)
    return

TABS.receiver.renderModel = ->
    if @keepRendering
    ...
    if RC.channels[0] and RC.channels[1] and RC.channels[2]
        ...
        roll = delta * @rateCurve.rcCommandRawToDegreesPerSecond(RC.channels[0], RC_tuning.roll_rate, RC_tuning.RC_RATE, RC_tuning.RC_EXPO, @useSuperExpo, @deadband, RC_tuning.roll_rate_limit)
        pitch = delta * @rateCurve.rcCommandRawToDegreesPerSecond(RC.channels[1], RC_tuning.pitch_rate, RC_tuning.rcPitchRate, RC_tuning.RC_PITCH_EXPO, @useSuperExpo, @deadband, RC_tuning.pitch_rate_limit)
        yaw = delta * @rateCurve.rcCommandRawToDegreesPerSecond(RC.channels[2], RC_tuning.yaw_rate, RC_tuning.rcYawRate, RC_tuning.RC_YAW_EXPO, @useSuperExpo, @yawDeadband, RC_tuning.yaw_rate_limit)
        @model.rotateBy -degToRad(pitch), -degToRad(yaw), -degToRad(roll)
    return

TABS.receiver.cleanup = (callback) ->
...
TABS.receiver.refresh = (callback) ->
    self = this
...
TABS.receiver.updateRcInterpolationParameters = ->
    if semver.gte(CONFIG.apiVersion, '1.20.0')
