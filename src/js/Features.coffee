...
Features = (config) ->
    ...
    if semver.gte(CONFIG.apiVersion, '1.15.0') and !semver.gte(CONFIG.apiVersion, '1.36.0')
        features.push
            bit: 8
            group: 'rxFailsafe'
            name: 'FAILSAFE'
...
Features::setMask = (featureMask) ->
    self = this
    self._featureMask = featureMask
    return

Features::isEnabled = (featureName) ->
    self = this
    i = 0
    while i < self._features.length
        if self._features[i].name == featureName and bit_check(self._featureMask, self._features[i].bit)
            return true
        i++
    false

Features::generateElements = (featuresElements) ->
    `var newElement`
    `var i`
    self = this
...
Features::updateData = (featureElement) ->
    `var bit`
    self = this
