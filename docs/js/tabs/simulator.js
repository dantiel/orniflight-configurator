...
TABS.simulator._initUI = function() {
  ...
  $('.sim-right input[type="range"]').on('input', function() {
    ...
    if (name === 'wing_pairs') {
      ...
      return self._applyWingGeometry();
    }
  });
...
TABS.simulator._readWingConfig = function() {
  var j, p, ref, ref1, ref2, ref3, self;
  self = this;
  self._pairCount = Math.max(1, Math.min(4, Math.round((ref = self.cfg.wing_pairs) != null ? ref : 2)));
  self._cg = ((ref1 = self.cfg.cg_position) != null ? ref1 : 0) / 100.0;
  self._mountAngle = [];
  self._mountDist = [];
  for (p = j = 0; j < 4; p = ++j) {
    self._mountAngle[p] = (ref2 = self.cfg['mount_angle_' + p]) != null ? ref2 : 0;
    self._mountDist[p] = ((ref3 = self.cfg['mount_distance_' + p]) != null ? ref3 : 0) / 100.0;
  }
};

TABS.simulator._applyWingGeometry = function() {
  var i, j, k, n, p, ref, ref1, ref2, self, t;
  self = this;
  if (!self.model) {
    return;
  }
...
TABS.simulator._initModel = function() {
  var canvas, e, fp, i, j, k, n, p, ref, ref1, ref2, ref3, self, t, waveEl, wrapper;
  self = this;
  self._slew = {
    freq: 0.0,
    freqMod: 1.0,
...
TABS.simulator._physicsStep = function() {
  var A_LAT, LIMIT, aileronDeg, ampScale, anchorBoost, authority, cg, cosT, ctrlPitch, ctrlRoll, ctrlYaw, d, dampBase, dampPitch, dampRoll, dampYaw, elevatorDeg, glideScale, j, lever, n, p, pitchCoef, pitchRank, pitchSum, rad, ref, ref1, ref2, ref3, ref4, rollCoef, rollCos, rudderDeg, self, sin30, sinT, th, yawCoef, yawMount, yawSin2, zEff;
  self = this;
  n = (ref = self._pairCount) != null ? ref : 2;
  A_LAT = 0.164;
